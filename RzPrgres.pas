{===============================================================================
  ...
  ------------------------------------------------------------------------------
  TRzProgressBar        Custom graphic control used to display progress
...
begin
  ...
  begin
    FBarStyle := Value;
    FShowPercent := FBarStyle = bsTraditional;
...
begin
  ...
  begin
    ...
    begin
      FPartsComplete := 0;                 { Set Internal FPartsComplete Field }
      Percent := 0;                                     { Set Percent Property }
    end;
  end;
end;
...
begin
  ...
  if ( Parent <> nil ) and ( Parent is TRzStatusBar ) then
    Result := Result and TRzStatusBar( Parent ).ThemeAware;
end;
...
begin
  ...
  try
    ...
    if FBarStyle = bsTraditional then
      DrawPercentBar( MemImage.Canvas, R, FOrientation, FBarColor, FBackColor, FPercent, FShowPercent )
    else
      DrawLEDBar( MemImage.Canvas, R, FOrientation, FBarColor, FBackColor, FNumSegments, FPercent, FThemeAware );
